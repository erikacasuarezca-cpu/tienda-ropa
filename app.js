...
/* ==========================================================
   REGISTRO (Mantener nombre tal cual lo ingresa el usuario)
=========================================================== */
btnRegister.onclick = async () => {
  let u = regUser.value.trim();
  u = u.charAt(0).toUpperCase() + u.slice(1).toLowerCase(); // Capitaliza la primera letra y el resto en minúsculas
  ...
  // limpiar: solo lower usuario
  users = users.map(row =>
    row.map((col, idx) => {
      if (typeof col === "string") {
        if (idx === 0) return col.trim(); // No tocar el nombre del usuario
        return col.trim(); // password nunca tocar mayúsculas
      }
      return col;
    })
  );
  ...
  await fetch(`${API_URL}?sheet=Usuarios`, {
    ...
    body: JSON.stringify({
      usuario: u,
      password: p, // clave corregida según Sheets
...
/* ==========================================================
   LOGIN (Mantener nombre tal cual lo ingresa el usuario)
=========================================================== */
btnLogin.onclick = async () => {
  let usr = loginUser.value.trim();
  usr = usr.charAt(0).toUpperCase() + usr.slice(1).toLowerCase(); // Capitaliza la primera letra y el resto en minúsculas
  let pass = loginPass.value.trim();

  let usersRaw = await fetch(`${API_URL}?sheet=Usuarios`);
  let users = await usersRaw.json();

  users = users.slice(1).map(row =>
    row.map((col, idx) => {
      if (typeof col === "string") {
        if (idx === 0) return col.trim(); // No tocar el nombre del usuario
        return col.trim();
  ...
  console.log("Usuarios desde Sheets (limpios):", users);

  // Real fix: comparar ambos como string
  let ok = users.find(row =>
    String(row[0]) === String(usr) && String(row[1]) === String(pass)
  );
...
function mostrarTienda(u) {
  ...
  nombreUsuario.textContent = u; // Muestra el nombre tal como lo ingresó el usuario
}
  ...
  productos.forEach(p => {
    adminDeleteList.innerHTML += `
      <div class="delItem">
        ...
        <button onclick="eliminarProd(${p.id})">❌</button>
      </div>
    `;
  });
}
...
async function eliminarProd(id) {
  await fetch(`${API_URL}?sheet=Productos&delete=${id}`);
  cargarProductos();
}
...
/* ==========================================================
   FINALIZAR COMPRA
=========================================================== */
btnSimularCompra.onclick = () =>
  modalPago.classList.remove("hidden");

btnCerrarPago.onclick = () =>
